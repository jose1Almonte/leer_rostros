"""RegistrarEncontradoSinImagen: flujo RESCATISTA sin foto (solo texto)."""
...
class RegistrarEncontradoSinImagen:
    """RESCATISTA sin imagen: registra una persona hallada por datos y devuelve, a la
    INVERSA, los familiares que la buscan que coinciden por texto (cédula/nombre)."""
    ...
    def execute(
        ...
        *,
        ...
        # Búsqueda INVERSA por TEXTO: familiares (buscadas visibles) que coinciden.
        buscadas = self._repo.buscar_por_texto(
            estado="buscada",
            nombre=nombre,
            apellido=apellido,
            doc_numero=doc_numero,
        ...
        total = self._repo.count_por_texto(
            estado="buscada", nombre=nombre, apellido=apellido, doc_numero=doc_numero
        )
        candidatos = [MenoresPrivacy(CandidatoTexto(**d)) for d in buscadas]
